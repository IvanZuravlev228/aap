...
export class MainComponent implements OnInit{
  ...
  public downloadSession(id: string, fileName: string) {
    ...
    this.http.get("http://91.207.183.142:8090/files/session/" + id, { responseType: 'blob', headers: headers, withCredentials: true })
      .subscribe((data: Blob) => {
...
// Создайте объект HttpHeaders с заголовком Authorization
    ...
    // Добавьте заголовки к запросу
    this.http.get("http://91.207.183.142:8090/files/auth/" + id, { responseType: 'blob', headers: headers, withCredentials: true })
      .subscribe((data: Blob) => {
        const downloadLink = document.createElement('a');
        const url = window.URL.createObjectURL(data);
        downloadLink.href = url;
